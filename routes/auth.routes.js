...
router.post("/signup", (req, res, next) => {
	const { email, password, name, coins } = req.body
	...
	User.findOne({ email })
		.then((foundUser) => {
			...
			return User.create({ email, password: hashedPassword, name, coins })
		})
		.then((createdUser) => {
			// Deconstruct the newly created user object to omit the password
			// We should never expose passwords publicly
			const { email, name, _id, coins } = createdUser

			// Create a new object that doesn't expose the password
			const user = { email, name, _id, coins }
